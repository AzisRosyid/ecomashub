...
@section('content')
    ...
    <div class="py-8 bg-gray-50 justify-start items-start w-full border-l">
        ...
        <!-- body -->
        <form action="{{ route('userAssetStore') }}" method="POST" class="mt-6 sm:mx-10 mx-2 font-fredokaRegular">
            @csrf
            <div class="border-b sm:flex">
                ...
                <input type="text" name="name" id="produk"
                    ...
                    placeholder="Nama aset" value="{{ old('name') }}" required>
            </div>
            ...
            <div class="border-b mt-3 sm:flex">
                ...
                <select name="unit_id" id="" class="outline-none border border-gray-400 p-2 rounded-lg mt-2 mb-4"
                    ...
                    @foreach ($units as $st)
                        <option value="{{ $st->id }}" {{ old('unit_id') == $st->id ? 'selected' : '' }}>
                            {{ $st->name }}
                        </option>
                    @endforeach
                </select>
