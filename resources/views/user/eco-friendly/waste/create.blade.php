...
@section('content')
    ...
    <div class="py-8 bg-gray-50 justify-start items-start w-full border-l">
        <a href="{{ route('userWaste') }}" class="mx-2 sm:mx-10 text-green-600 text-sm font-medium flex">
        ...
        <div class="flex mx-2 sm:mx-10 justify-between border-b pb-4 mt-4">
            <div>
                <p class="text-zinc-700 text-[28px] font-semibold font-fredokaBold leading-9">Tambah Sampah</p>
        ...
        <!-- body -->
        <form action="{{ route('userWasteStore') }}" method="POST" class="mt-6 sm:mx-10 mx-2 font-fredokaRegular">
            @csrf
            <div class="border-b sm:flex">
                ...
                <input type="text" name="name" id="produk"
                    ...
                    placeholder="Nama sampah" value="{{ old('name') }}" required>
            </div>
            ...
            <div class="border-b mt-3 sm:flex">
                ...
                <select name="product_id" id=""
                    ...
                    @foreach ($products as $st)
                        <option value="{{ $st->id }}" {{ old('product_id') == $st->id ? 'selected' : '' }}>
                            {{ $st->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="border-b mt-3 sm:flex">
                ...
                <select name="type_id" id="" class="outline-none border border-gray-400 p-2 rounded-lg mt-2 mb-4"
                    ...
                    @foreach ($types as $st)
                        <option value="{{ $st->id }}" {{ old('type_id') == $st->id ? 'selected' : '' }}>
                            {{ $st->name }}
                        </option>
                    @endforeach
                </select>
