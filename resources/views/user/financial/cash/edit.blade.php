...
@section('content')
    ...
    <div class="py-8 bg-gray-50 justify-start items-start w-full border-l">
        <a href="{{ route('adminCash') }}" class="mx-2 sm:mx-10 text-green-600 text-sm font-medium flex">
        ...
        <div class="flex mx-2 sm:mx-10 justify-between border-b pb-4 mt-4">
            <div>
                <p class="text-zinc-700 text-[28px] font-semibold font-fredokaBold leading-9">Edit Kas</p>
        ...
        <!-- body -->
        <form action="{{ route('adminCashUpdate', $cash) }}" method="POST" class="mt-6 sm:mx-10 mx-2 font-fredokaRegular">
            @method('put')
            @csrf
            <div class="border-b sm:flex">
                ...
                <input type="text" name="name" id="judulHutang"
                    ...
                    value="{{ old('name') ?? $cash->name }}" required>
            </div>
            <div class="border-b mt-3 sm:flex">
                ...
                <textarea name="description" id="deskripsi" cols="30" rows="3"
                    class="outline-none border border-gray-400 p-2 rounded-lg mt-2 mb-4 sm:w-[600px]" placeholder="Deskripsi">{{ old('description') ?? $cash->description }}</textarea>
            </div>
            <div class="border-b mt-3 sm:flex">
                ...
                <select name="supplier_id" id=""
                    ...
                    @foreach ($collaborations as $st)
                        <option value="{{ $st->id }}"
                            {{ (old('collaboration_id') ?? $cash->collaboration_id) === $st->id ? 'selected' : '' }}>
                            {{ $st->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="border-b mt-3 sm:flex">
                ...
                <input type="number" name="value" id="nilai"
                    ...
                    value="{{ old('value') ?? $cash->value }}" required>
            </div>
            <div class="border-b mt-3 sm:flex">
                    ...
                    @foreach ($types as $index => $st)
                        <div class="flex @if ($index > 0) ms-6 @endif">
                            <input type="radio" name="type" class="cash-type" value="{{ $st }}"
                                ...
                                @if ((old('type') ?? $cash->type) === $st) checked @endif>
            ...
            <div class="border-b mt-3 sm:flex">
                <label for="interval" id="intervalLabel"
                    class="sm:w-40 block">{{ (old('type') ?? $cash->type) === 'Rutin' ? 'Interval (bulan)*' : 'interval (bulan)' }}</label>
                <input type="number" name="interval" id="interval"
                    ...
                    {{ (old('type') ?? $cash->type) === 'Rutin' ? 'required' : 'disabled' }}
                    value="{{ old('interval') ?? $cash->interval }}">
            </div>
            <div class="border-b mt-3 sm:flex">
                ...
                <div class="flex sm:w-[600px]">
                    <div>
                        ...
                        <input type="datetime-local" name="date_start"
                            ...
                            value="{{ old('date_start') ?? $cash->date_start }}" required>
                    </div>
                    <div class="ms-6">
                        <label for="selesai" class="block"
                            id="dateEndLabel">{{ (old('type') ?? $cash->type) === 'Rutin' ? 'Tanggal selesai*' : 'Tanggal selesai' }}</label>
                        <input type="datetime-local" name="date_end" id="dateEnd"
                            class="outline-none border border-gray-400 p-2 rounded-lg mt-2 mb-4"
                            value="{{ old('date_end') ?? $cash->date_end }}"
                            {{ (old('type') ?? $cash->type) === 'Rutin' ? 'required' : 'disabled' }}>
                    </div>
                </div>
            </div>
            <div class="flex justify-end mt-4">
                <a href="{{ route('adminCash') }}"
