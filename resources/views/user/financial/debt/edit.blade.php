...
@section('content')
    ...
    <div class="py-8 bg-gray-50 justify-start items-start w-full border-l">
        <a href="{{ route('userDebt') }}" class="mx-2 sm:mx-10 text-green-600 text-sm font-medium flex">
        ...
        <div class="flex mx-2 sm:mx-10 justify-between border-b pb-4 mt-4">
            <div>
                <p class="text-zinc-700 text-[28px] font-semibold font-fredokaBold leading-9">Edit Hutang</p>
        ...
        <!-- body -->
        <form action="{{ route('userDebtStore') }}" method="POST" class="mt-6 sm:mx-10 mx-2 font-fredokaRegular">
            @csrf
            ...
            <div class="border-b mt-3 sm:flex">
                ...
                <textarea name="description" id="deskripsi" cols="30" rows="3"
                    class="outline-none border border-gray-400 p-2 rounded-lg mt-2 mb-4 sm:w-[600px]" placeholder="Deskripsi">{{ old('description') ?? $debt->description }}</textarea>
            </div>
            <div class="border-b mt-3 sm:flex">
                ...
                <input type="number" name="value" id="nilai"
                    ...
                    value="{{ old('value') ?? $debt->value }}" required>
            </div>
            <div class="border-b mt-3 sm:flex">
                <label for="bunga" class="sm:w-40 block">Bunga (%)*</label>
                <input type="number" name="interest" id="bunga"
                    class="sm:w-[600px] outline-none border border-gray-400 p-2 rounded-lg mt-2 mb-4" placeholder="Bunga"
                    value="{{ old('interest') ?? $debt->interest * 100 }}" required>
            </div>
            <div class="border-b mt-3 sm:flex">
                ...
                <div class="flex sm:w-[600px]">
                    <div>
                        ...
                        <input type="datetime-local" name="date_start"
                            ...
                            value="{{ old('date_start') ?? $debt->date_end }}" required>
                    </div>
                    <div class="ms-6">
                        <label for="selesai" class="block">Tanggal selesai*</label>
                        <input type="datetime-local" name="date_end"
                            class="outline-none border border-gray-400 p-2 rounded-lg mt-2 mb-4"
                            value="{{ old('date_end') ?? $debt->date_end }}" required>
                    </div>
                </div>
            </div>
            <div class="flex justify-end mt-4">
                <a href="{{ route('userDebt') }}"
