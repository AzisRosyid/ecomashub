...
@section('content')
    ...
    <div class="py-8 bg-gray-50 justify-start items-start w-full border-l">
        <a href="{{ route('userExpense') }}" class="mx-2 sm:mx-10 text-green-600 text-sm font-medium flex">
        ...
        <div class="flex mx-2 sm:mx-10 justify-between border-b pb-4 mt-4">
            <div>
                <p class="text-zinc-700 text-[28px] font-semibold font-fredokaBold leading-9">Tambah Biaya</p>
        ...
        <!-- body -->
        <form action="{{ route('userExpenseStore') }}" method="POST" class="mt-6 sm:mx-10 mx-2 font-fredokaRegular">
            @csrf
            <div class="border-b sm:flex">
                ...
                <input type="text" name="name" id="judulHutang"
                    ...
                    placeholder="Nama biaya" value="{{ old('name') }}" required>
            </div>
            ...
            <div class="border-b mt-3 sm:flex">
                <label for="" class="sm:w-40 block">Pemasok</label>
                <select name="collaboration_id" id=""
                    class="outline-none border border-gray-400 p-2 rounded-lg mt-2 mb-4" placeholder="Pilih collaboration">
            ...
            <div class="border-b mt-3 sm:flex">
                <label for="interval" id="intervalLabel"
                    class="sm:w-40 block">{{ old('type') == 'Rutin' ? 'Interval (bulan)*' : 'interval (bulan)' }}</label>
                <input type="number" name="interval" id="interval"
                    ...
                    {{ old('type') == 'Rutin' ? 'required' : 'disabled' }} value="{{ old('interval') }}">
            </div>
            <div class="border-b mt-3 sm:flex">
