...
@section('content')
    ...
    <div class="py-8 bg-gray-50 justify-start items-start w-full border-l">
        <a href="{{ route('adminCollaboration') }}" class="mx-2 sm:mx-10 text-green-600 text-sm font-medium flex">
        ...
        <div class="flex mx-2 sm:mx-10 justify-between border-b pb-4 mt-4">
            <div>
                <p class="text-zinc-700 text-[28px] font-semibold font-fredokaBold leading-9">Edit Kolaborasi</p>
        ...
        <!-- body -->
        <form action="{{ route('adminCollaborationUpdate', $collaboration) }}" method="POST"
            ...
            <div class="border-b mt-3 sm:flex">
                ...
                <input type="text" name="name" id="nama"
                    ...
                    placeholder="Nama pemasok" value="{{ old('name') ?? $collaboration->name }}" required>
            </div>
            <div class="border-b mt-3 sm:flex">
                <label for="deskripsi" class="sm:w-40 block">Deskripsi</label>
                <textarea name="description" id="deskripsi" cols="30" rows="3"
                    class="outline-none border border-gray-400 p-2 rounded-lg mt-2 mb-4 sm:w-[600px]" placeholder="Deskripsi">{{ old('description') ?? $collaboration->description }}</textarea>
            </div>
            <div class="border-b mt-3 sm:flex">
                    ...
                    @foreach ($types as $index => $st)
                        <div class="flex @if ($index > 0) ms-6 @endif">
                            <input type="radio" name="type" value="{{ $st }}" id="{{ strtolower($st) }}"
                                ...
                                @if ((old('type') ?? $collaboration->type) === $st) checked @endif>
            ...
            <div class="border-b mt-3 sm:flex">
                ...
                <input type="email" name="email" id="email"
                    ...
                    value="{{ old('email') ?? $collaboration->email }}" required>
            </div>
            <div class="border-b mt-3 sm:flex">
                ...
                <input type="number" id="nomor"
                    ...
                    value="{{ old('phone_number') ?? $collaboration->phone_number }}" required>
            </div>
            <div class="border-b mt-3 sm:flex">
                ...
                <input type="text" id="alamat"
                    ...
                    name="address" value="{{ old('address') ?? $collaboration->address }}" required>
            </div>
                    ...
                    @foreach ($status as $index => $st)
                        <div class="flex @if ($index > 0) ms-6 @endif">
                            <input type="radio" name="status" value="{{ $st }}" id="{{ strtolower($st) }}"
                                @if ($index === 0) required @endif
                                @if ((old('status') ?? $collaboration->status) === $st) checked @endif>
                            <label for="{{ strtolower($st) }}" class="ms-2">{{ $st }}</label>
                        </div>
                    @endforeach
                </div>
