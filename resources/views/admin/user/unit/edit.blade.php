...
@section('content')
    ...
    <div class="py-8 bg-gray-50 justify-start items-start w-full border-l">
        ...
        <div class="flex mx-2 sm:mx-10 justify-between border-b pb-4 mt-4">
            <div>
                <p class="text-zinc-700 text-[28px] font-semibold font-fredokaBold leading-9">Edit Peran Pengguna</p>
        ...
        <!-- body -->
        <form action="{{ route('adminUserRoleUpdate', $role) }}" method="POST"
            ...
            <div class="border-b mt-3 sm:flex">
                ...
                <input type="text" name="name" id="nama"
                    ...
                    placeholder="Nama peran pengguna" value="{{ old('name') ?? $role->name }}" required>
            </div>
            <div class="border-b mt-3 sm:flex">
                    ...
                    @foreach ($types as $index => $st)
                        <div class="flex @if ($index > 0) ms-6 @endif">
                            <input type="radio" name="type" value="{{ $st }}" id="{{ strtolower($st) }}"
                                ...
                                @if (old('type') ?? $role->type == $st) checked @endif>
