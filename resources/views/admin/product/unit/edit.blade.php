...
@section('content')
    ...
    <div class="py-8 bg-gray-50 justify-start items-start w-full border-l">
        <a href="{{ route('adminProduct') }}" class="mx-2 sm:mx-10 text-green-600 text-sm font-medium flex">
        ...
        <div class="flex mx-2 sm:mx-10 justify-between border-b pb-4 mt-4">
            <div>
                <p class="text-zinc-700 text-[28px] font-semibold font-fredokaBold leading-9">Edit Kategori Produk</p>
        ...
        <!-- body -->
        <form action="{{ route('adminProductCategoryUpdate', $category) }}" method="POST"
            class="mt-6 sm:mx-10 mx-2 font-fredokaRegular">
            @method('put')
            @csrf
            <div class="border-b mt-3 sm:flex">
                <label for="nama" class="sm:w-40 block">Nama kategori*</label>
            ...
            <div class="flex justify-end mt-4">
                <a href="{{ route('adminProduct') }}"
                    ...
                    Batal</a>
                <button
                    class="px-4 h-10 rounded-lg border border-amber-400 bg-amber-400 text-white text-sm font-normal font-fredokaRegular items-center flex ms-3">Edit
