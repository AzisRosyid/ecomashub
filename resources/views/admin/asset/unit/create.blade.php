...
@section('content')
    ...
    <div class="py-8 bg-gray-50 justify-start items-start w-full border-l">
        ...
        <div class="flex mx-2 sm:mx-10 justify-between border-b pb-4 mt-4">
            <div>
                <p class="text-zinc-700 text-[28px] font-semibold font-fredokaBold leading-9">Tambah Satuan Aset</p>
        ...
        <!-- body -->
        <form action="{{ route('adminAssetUnitStore') }}" method="POST" class="mt-6 sm:mx-10 mx-2 font-fredokaRegular">
            @csrf
            <div class="border-b mt-3 sm:flex">
                <label for="nama" class="sm:w-40 block">Nama satuan*</label>
                <input type="text" name="name" id="nama"
                    class="sm:w-[600px] outline-none border border-gray-400 p-2 rounded-lg mt-2 mb-4"
                    placeholder="Nama satuan aset" value="{{ old('name') }}" required>
            </div>
