...
if (selectStore) {
    selectStore.addEventListener('change', function () {
        return new Promise((resolve, reject) => {
            try {
                fetch(window.location.origin + `/store/?api&select=${this.value}`)
...
if (productModal != null) {
    ...
    const loadModal = () => {
        return new Promise((resolve, reject) => {
            try {
                clearModal().then(s => fetch(window.location.origin + `/user/product?api&search=${productModalSearch.value}&pick=${productModalPick.value}&page=${productPage}`)
                    .then(response => response.json())
                    .then(data => {
